...
void main() {
	...
	float splat = 1.0;//texture(splatmap, mapPosition).r;
	
	vec3 dirtColor = mix(dirt1Color, dirt2Color, splat);
	vec3 dirtNormal = mix(dirt1Normal, dirt2Normal, splat);

	vec3 stoneColor = mix(stone1Color, stone2Color, splat) * 0.6;
	vec3 stoneNormal = mix(stone1Normal, stone2Normal, splat);
	...
	ps_position = vec4((gs_position / 1000.0 + 1.0) / 2.0, 1.0); 
