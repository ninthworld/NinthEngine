...
ResourceManager::ResourceManager(Graphics* graphics) : m_graphics(graphics) {}
...
Model* ResourceManager::loadModel(const std::string& file) {
	...
	for (size_t s = 0; s < shapes.size(); ++s) {
		...
		Mesh* mesh = createMesh(meshProps);
	...
	Model* model = createModel(modelProps);
...
Shader* ResourceManager::loadShader(const std::string& vertexShader, const std::string& pixelShader) {
	ShaderProps props;
...
Model* ResourceManager::createModel(const ModelProps& props) {
	Model* model = new Model(props);
	m_models.push_back(model);
	return model;
}

Mesh* ResourceManager::createMesh(const MeshProps& props) {
	Mesh* mesh = new Mesh(props);
