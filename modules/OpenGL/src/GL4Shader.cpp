...
void GL4Shader::bind() {
	...
	for (auto it = m_textures.begin(); it != m_textures.end(); ++it) {
		...
		if (sampler) {
			auto glSampler = (GL4Sampler*)sampler;
...
void GL4Shader::unbind() {
	...
	GLuint nativeId = 0;
	GL4Shader* shader = m_popShader();
	if (shader != nullptr) nativeId = shader->getNativeId();
	glUseProgram(nativeId);
}

void GL4Shader::addConstantBuffer(const unsigned int index, ConstantBuffer* buffer) {
	auto glBuffer = (GL4ConstantBuffer*)buffer;
	m_constantBuffers.insert(std::make_pair(index, glBuffer));
}

void GL4Shader::addTexture(const unsigned int index, Texture* texture) {
